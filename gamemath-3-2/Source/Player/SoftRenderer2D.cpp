...
void SoftRenderer::Render2D()
{
	...
	static constexpr float radiusradius = radius * radius;
	...
	if (circles.empty())
	{
		for (float x = -radius; x <= radius; ++x)
		{
			for (float y = -radius; y <= radius; ++y)
			{
				...
				const float squaredLength = pointToTest.SizeSquared();
				if (squaredLength <= radiusradius)
				{
					circles.emplace_back(Vector2(x, y));
				}
			}
		}
	}
