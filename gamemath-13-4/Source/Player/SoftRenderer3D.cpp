...
void SoftRenderer::Render3D()
{
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		const Matrix4x4 finalTransposedMatrix = finalMatrix.Transpose();
		const std::array<Plane, 6> frustumPlanesFromMatrix =
		{
		...
		const Frustum frustumFromMatrix(frustumPlanesFromMatrix);

		// 바운딩 영역을 사용해 절두체 컬링을 구현
		Sphere sphereBound = mesh.GetSphereBound();
		const BoundCheckResult checkResult = frustumFromMatrix.CheckBound(sphereBound);
