...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	GameObject& goEart = g.GetGameObject(EarthGo);
	GameObject& goMoon = g.GetGameObject(MoonGo);

	// 각 행성에 회전 부여
	goSun.GetTransform().AddLocalYawRotation(rotateSpeedSun * InDeltaSeconds);
	goEart.GetTransform().AddLocalYawRotation(rotateSpeedEarth * InDeltaSeconds);
	goMoon.GetTransform().AddLocalYawRotation(rotateSpeedMoon * InDeltaSeconds);

	// 카메라를 움직이되 카메라가 항상 태양을 바라보도록 설정
	const Vector3 inputVector = Vector3(input.GetAxis(InputAxis::XAxis), input.GetAxis(InputAxis::YAxis), input.GetAxis(InputAxis::ZAxis)).GetNormalize();
