...
void SoftRenderer::Render3D()
{
	...
	const float nearZ = mainCamera.GetNearZ();
	const float farZ = mainCamera.GetFarZ();
	const float halfFOV = mainCamera.GetFOV() * 0.5f;
	float pSin = 0.f, pCos = 0.f;
	Math::GetSinCos(pSin, pCos, halfFOV);

	// 절두체를 구성하는 평면의 방정식
	static const std::array<Plane, 6> frustumPlanes =
	{
	...
	// 절두체 선언
	const Frustum frustumInView(frustumPlanes);

	// 절두체 컬링 테스트를 위한 통계 변수
	const size_t totalObjects = g.GetScene().size();
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		// 절두체 컬링 구현
		const Vector4 viewPos = vMatrix * Vector4(transform.GetPosition());
