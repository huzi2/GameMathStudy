...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	// 게임 로직의 로컬 변수
	static const float moveSpeed = 500.f;
	static const float fovSpeed = 100.f;
	static const float minFOV = 15.f;
	...
	const float deltaFOV = input.GetAxis(InputAxis::WAxis) * fovSpeed * InDeltaSeconds;
...
void SoftRenderer::Render3D()
{
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		const Matrix4x4 finalMatrix = pMatrix * vMatrix * transform.GetModelingMatrix();
...
void SoftRenderer::DrawTriangle3D(std::vector<Vertex3D>& InVertices, const LinearColor& InColor, FillMode InFillMode)
{
	...
	for (Vertex3D& v : InVertices)
	{
		...
		const float invZ = 1.f / v.Position.Z;
		v.Position.X *= invZ;
		v.Position.Y *= invZ;
		v.Position.Z *= invZ;
	}
	...
	// 백페이스 컬링
	const Vector3 edge1 = (InVertices[1].Position - InVertices[0].Position).ToVector3();
	const Vector3 edge2 = (InVertices[2].Position - InVertices[0].Position).ToVector3();
	const Vector3 faceNormal = -edge1.Cross(edge2);
	const Vector3 viewDirection = Vector3::UnitZ;
