...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
    ...
    const Vector2 playerPos = transform.GetPosition();
    const Vector2 cameraPos = cameraTransform.GetPosition();
    ...
    {
        const float ratio = Math::Clamp(lerpSpeed * InDeltaSeconds, 0.f, 1.f);
        const Vector2 newCameraPos = cameraPos + (playerPos - cameraPos) * ratio;
...
void SoftRenderer::Render2D()
{
    ...
    const Matrix3x3 viewMatrix = g.GetMainCamera().GetViewMatrix();
