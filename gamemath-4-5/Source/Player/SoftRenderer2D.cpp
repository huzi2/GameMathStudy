...
void SoftRenderer::Render2D()
{
	...
	static const float maxLength = Vector2(_ScreenSize.X, _ScreenSize.Y).Size() * 0.5f;
	...
	for (const auto& v : squares)
	{
		...
		const float ratio = polarV.X / maxLength;
		const float weight = Math::Lerp(1.f, 5.f, ratio);

		// 극좌표계를 사용해 회전을 부여한다.
		polarV.Y += Math::Deg2Rad(currentDegree) * weight;

		// 최종 값을 데카르트 좌표계로 변환한다.
		const Vector2 cartesianV = polarV.ToCartesianCoordinate();
