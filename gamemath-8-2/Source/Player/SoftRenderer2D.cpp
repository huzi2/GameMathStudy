...
void SoftRenderer::Render2D()
{
	...
	{
		size_t bi = ti * 3;
		const std::array<Vertex2D, 3> tv = { vertices[indices[bi]], vertices[indices[bi + 1]], vertices[indices[bi + 2]] };

		const Vector2 minPos(Math::Min3(tv[0].Position.X, tv[1].Position.X, tv[2].Position.X), Math::Min3(tv[0].Position.Y, tv[1].Position.Y, tv[2].Position.Y));
		const Vector2 maxPos(Math::Max3(tv[0].Position.X, tv[1].Position.X, tv[2].Position.X), Math::Max3(tv[0].Position.Y, tv[1].Position.Y, tv[2].Position.Y));

		// 무게중심좌표를 위한 준비작업
		const Vector2 u = tv[1].Position - tv[0].Position;
		const Vector2 v = tv[2].Position - tv[0].Position;

		// 공통분모 (uu * vv - uv * uv)
		const float udotv = u.Dot(v);
		const float vdotv = v.Dot(v);
		const float udotu = u.Dot(u);
		const float denominator = udotv * udotv - vdotv * udotu;
		...
		for (int x = lowerLeftPoint.X; x <= upperRightPoint.X; ++x)
		{
			for (int y = upperRightPoint.Y; y <= lowerLeftPoint.Y; ++y)
			{
				...
				const Vector2 pointToTest = fragment.ToCartesianCoordinate(_ScreenSize);
				const Vector2 w = pointToTest - tv[0].Position;
				const float wdotu = w.Dot(u);
				const float wdotv = w.Dot(v);

				// 분자 값을 구하고 최종 무게중심좌표 산출
				const float s = (wdotv * udotv - wdotu * vdotv) * invDenominator;
				const float t = (wdotu * udotv - wdotv * udotu) * invDenominator;
				const float oneMinusST = 1.f - s - t;
