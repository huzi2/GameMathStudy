...
void SoftRenderer::LateUpdate3D(float InDeltaSeconds)
{
	...
	// 애니메이션을 위한 커브 생성
	const float neckCurrent = Math::FMod(elapsedTime, neckLength) * Math::TwoPI / neckLength;
	const float armLegCurrent = Math::FMod(elapsedTime, armLegLength) * Math::TwoPI / armLegLength;

	const float neckCurve = sinf(neckCurrent) * neckDegree;
	const float armLegCurve = sinf(armLegCurrent) * armLegDegree;
	...
	for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
	{
		...
		if (mesh.IsSkinnedMesh() && IsWireframeDrawing())
		{
			...
			for (const auto& b : mesh.GetBones())
			{
				...
				const Vector3 boneVector = wt2.GetPosition() - wt1.GetPosition();
				const Transform tboneObject(wt1.GetPosition(), Quaternion(boneVector), Vector3(10.f, 10.f, boneVector.Size()));
				const Matrix4x4 boneMatrix = pvMatrix * tboneObject.GetMatrix();
...
void SoftRenderer::DrawMesh3D(const Mesh& InMesh, const Matrix4x4& InMatrix, const LinearColor& InColor)
{
	...
	for (size_t vi = 0; vi < vertexCount; ++vi)
	{
		...
		if (InMesh.IsSkinnedMesh())
		{
			...
			const Weight& w = InMesh.GetWeights()[vi];
			for (size_t wi = 0; wi < InMesh.GetConnectedBones()[vi]; ++wi)
			{
				const std::string& boneName = w.Bones[wi];
				if (InMesh.HasBone(boneName))
				{
					...
					const Transform boneLocal = t.WorldToLocal(bindPose);

					// 점의 위치를 바인드포즈를 중심으로 재해석
					const Vector3 localPosition = bindPose.WorldToLocalVector(vertices[vi].Position.ToVector3());

					// 점에 본의 로컬 트랜스폼을 적용해 바인드포즈로부터 변화된 값을 반영
					const Vector3 skinnedLocalPosition = boneLocal.GetMatrix() * localPosition;
					// 점의 위치를 모델링 공간으로 다시 변경
					const Vector3 skinnedWorldPosition = bindPose.GetMatrix() * skinnedLocalPosition;
