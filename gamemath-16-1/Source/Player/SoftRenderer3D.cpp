...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	if (elapsedTime == duration)
	{
		...
		const Vector3 randomAxis = Vector3(dir(generator), dir(generator), dir(generator)).GetNormalize();
	...
	else
	{
		...
		const Quaternion current = Quaternion::Slerp(startRotation, endRotation, t);
