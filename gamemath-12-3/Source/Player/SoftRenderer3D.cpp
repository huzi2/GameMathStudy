...
void SoftRenderer::DrawTriangle3D(std::vector<Vertex3D>& InVertices, const LinearColor& InColor, FillMode InFillMode)
{
	...
	else
	{
		...
		// 각 정점마다 보존된 뷰 공간의 z값
		const float invZ0 = 1.f / InVertices[0].Position.W;
		const float invZ1 = 1.f / InVertices[1].Position.W;
		const float invZ2 = 1.f / InVertices[2].Position.W;
		...
		for (int x = lowerLeftPoint.X; x <= upperRightPoint.X; ++x)
		{
			for (int y = upperRightPoint.Y; y <= lowerLeftPoint.Y; ++y)
			{
				...
				{
					const float z = invZ0 * oneMinusST + invZ1 * s + invZ2 * t;
					const float invZ = 1.f / z;
					const Vector2 targetUV = (InVertices[0].UV * oneMinusST * invZ0 + InVertices[1].UV * s * invZ1 + InVertices[2].UV * t * invZ2) * invZ;
